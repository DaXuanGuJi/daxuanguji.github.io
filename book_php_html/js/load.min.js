...
var fliespt = "#_", otxt = "", spdf = function (a) {
	...
	return br
}, afg = 1, bfg = 1, tdh = 0, tlh = 0, thy = 0, tflg = 0, lastflg = 0, imgid = 0, nullpage = 0,sendName =localStorage.sessionGUID?localStorage.sessionGUID:"null", sendclick = function (a) {
	if(((new Date).getTime()-nowTime)<5000)return;
	nowTime=(new Date).getTime();
	alertbox(360, 360, "\u8fdb\u5ea6:", '\x3cdiv id\x3d"logdiv" style\x3d"width:360px;height:336px;overflow-x:hidden;"\x3e\x3c/div\x3e', "read", 1);
	a = a.replace(/;(\\t)+/g, ";");
	a = a.replace(/[\t\r\n]+/g, "");
	ByID("logdiv").innerHTML = sendExeUrlStatu+"<br>";
	var loadtimer=function(node){node.innerHTML+="。";if(!ByID("retimg")){setTimeout(function (){loadtimer(node);},2000)}}
	var c = function (a) {
		a.onload = function (b) {
			b.stopPropagation();
			b.preventDefault();
			if((!a.naturalHeight)||(!a.naturalWidth))return;
			this.setAttribute("crossOrigin", "anonymous");
			canvs.width = this.naturalWidth;
			canvs.height = this.naturalHeight;
			canv.drawImage(this, 0, 0, canvs.width, canvs.height);	
			var tidx=parseInt(a.getAttribute('tidx'));
			var pidx=parseInt(a.getAttribute('pidx'));
			if(tidx&&(tidx<lput.value.length-1)){
				if(ByID("upspantext"))return;
				var dv=document.createElement("div");
				dv.className='btnb';
				dv.innerHTML='更新下一页文本';
				dv.id="upspantext";
				dv.onclick=function (e) {
					if(/已更新/.test(this.innerHTML))return;
					this.innerHTML='下一页文本 >>> 已更新';
					lput.value=lput.value.slice(tidx);
					lput.value=lput.value.replace(/^\${1,4}[\|\*]/,"");
					this.style.color="#f00";			
				}
				ByID("logdiv").insertBefore(dv,ByID("retimg"));
				ByID("retimg").setAttribute('tidx','0');
			}
			if(pidx&&(pidx<pyput.value.length-1)){
				if(ByID("upspanpy"))return;
				var dv=document.createElement("div");
				dv.className='btnb';
				dv.innerHTML='更新下一页拼音';
				dv.id="upspanpy";
				dv.onclick=function (e) {
					if(/已更新/.test(this.innerHTML))return;
					this.innerHTML='下一页拼音 >>> 已更新';	
					var pytxt=pyput.value;
					pytxt=pytxt.split(",");
					pyput.value=pytxt.slice(pidx);	
					this.style.color="#f00";			
				}				
				ByID("logdiv").insertBefore(dv,ByID("retimg"));
				ByID("retimg").setAttribute('pidx','0');
			}
			...
			try {
				if (200 === b.status)
					if (txt = b.responseText, lock = 0, /path\$\?/.test(txt)) {
						var a = "序列任务正在进行。。。程序锁定中<br>",
						g = txt.replace("path$?", ""),
						a = a + (g + "<br>待此任务完成后再试。。。<br>");
						ByID("logdiv").title = g;
						a += '<span id="logdivck" class="btnb">查看此任务</span><br>';
						g = sendExeUrl+'get.php?t='+(new Date).getTime()+'&img=./' + g;
									...
									if (4 === d.readyState)
										try {
											if (200 === d.status) {
												...
												ByID("logdiv").scrollTop = ByID("logdiv").scrollHeight;												
												if (/json\.png[\r\n]*/.test(e)) {													
													ByID("logdiv").innerHTML += 'Image output is ok:<br><image id="retimg" width=340 src="'+ g + '.min.png">';
											...
											ByID("logdiv").innerHTML +="<br>⛔服务器连接故障。。。<br>"+m;
										}

								};
								d.open("GET", sendExeUrl + "get.php?log=" + ByID("logdiv").title + ".log&setflg=temp&t=" + (new Date).getTime(), !0);
								d.send()
							};
							if(!ByID("logdiv").getAttribute('load'))b(0);
							ByID("logdiv").setAttribute('load','1');
						}
					} else{
						ByID("logdiv").innerHTML = txt, ByID("logdiv").scrollTop = ByID("logdiv").scrollHeight;
						if(4 === b.readyState&&200 === b.status){
							if(/GUID已失效/.test(txt))window.useGUID="null";
							if(ByID("retimg"))c(ByID("retimg"));
						}
						if(ByID("rettime"))loadtimer(ByID("rettime"));
					}
				else
					b.status && alert("\u51fa\u9519\n\u7f51\u5740\u672a\u8fde\u901a:" + b.status)
			} catch (e) {
				ByID("logdiv").innerHTML +="<br>⛔服务器连接故障。。。<br>";
			}

	};
	var ug=localStorage.useGUID?localStorage.useGUID:"null";
	if(ug!=="null"){
		if(!confirm('是否使用GUID提交数据?\n确定将会生成300dpi高清图片；GUID使用次数-1\n取消将会生成48dpi网络图片；今日IP使用次数-1')){ug="null"}
	}
	//b.withCredentials=true;
	b.timeout = 0;
	b.ontimeout = function () {};
	b.open("POST", sendExeUrl + "chromeDriver_headerless.php?guid="+ug+"&t=" + (new Date).getTime(), !0);
		b.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	b.send("ptsetmp\x3d" + new Base64().encode(a) + "\x26end\x3dChromeDiver\x26host\x3d" + sendExeUrl + "\x26txt\x3d" + sendName + "\x26time\x3d" + (new Date).getTime())
}, onbody = function () {
	boxdiv.innerHTML = "";
	uflg = Number(ptset.bodyobj.trand);
