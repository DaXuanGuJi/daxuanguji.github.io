...
document.body.onload = function (b) {
	...
	A = function (a) {
		...
		try {
			...
			case "tbodyrand":
				...
				if(a.value=='0')alert("本地字体需选择本地字体目录【读取文本前的下拉列表选择】,若目录无该字图片无法显示");
	...
	stuc.onclick = function (a) {
		window.win = window.open("https://daxuanprint.github.io/", "_blank", "top\x3d" + (screen.availHeight - 200) / 2 + ",left\x3d" + (screen.availWidth - 400) / 2 + ",width\x3d400,height\x3d200,directories\x3dno,status\x3dno,location\x3dno,toolbar\x3dno,scrollbars\x3dyes,resizable\x3dno,hotkeys\x3dno");
		setTimeout(function () {
	...
	stub.onclick = function (a) {
		a = prompt("\u8bf7\u8f93\u5165\u67e5\u627e\u4e66\u540d!", "\u897f\u6e38\u8bb");
		var d = screen.availWidth / 2,
		b = screen.availHeight,
		e = parseInt((screen.availHeight - b) / 2 - b),
		f = parseInt(screen.availWidth - d);
		window.open("https://daxuanprint.github.io/search.html?key=" + a, "wjss", "top\x3d" + e + ",left\x3d" + f + ",width\x3d" + d + ",height\x3d" + b + ",directories\x3dno,status\x3dno,location\x3dno,toolbar\x3dno,scrollbars\x3dyes,resizable\x3dno,hotkeys\x3dno")
	};
